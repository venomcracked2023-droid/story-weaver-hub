...
export const Route = createFileRoute("/truyen/$slug/chuong-{$num}")({
  component: Reader,
  ssr: false,
  loader: async ({ params }) => {
    ...
    const { data: comic } = await supabase
      ...
      .eq("slug", params.slug)
      .maybeSingle();
    if (!comic) return { comicTitle: null, coverId: null, chapterTitle: null };
    const { data: chapters } = await supabase
      .from("chapters")
      .select("title,order_index")
      .eq("comic_id", comic.id)
      .order("order_index", { ascending: true });
    const ch = (chapters ?? [])[n - 1] ?? null;
    return {
  ...
  head: ({ loaderData, params }) => {
    ...
    const url = `${SITE_URL}/truyen/${params.slug}/chuong-${params.num}`;
...
function Reader() {
  const { slug, num } = Route.useParams();
  const navigate = useNavigate();
  const comics = useComics();
  const loaded = useComicsLoaded();
  const comic = comics.find((c) => c.slug === slug);
  const idx = Math.max(0, parseInt(num, 10) - 1);
  const chapter = comic && idx >= 0 && idx < comic.chapters.length ? comic.chapters[idx] : null;
  ...
  const next = idx < comic.chapters.length - 1 ? idx + 2 : null;
  ...
  const goToNum = (n: number) =>
    navigate({
      ...
      params: { slug: comic.slug, num: String(n) },
    });
    ...
    >
      ...
      <div className="mx-auto flex max-w-3xl items-center gap-2 px-3 py-2 sm:px-4">
        ...
        <div className="relative min-w-0 flex-1">
          ...
          <select
            value={String(idx + 1)}
            onChange={(e) => goToNum(parseInt(e.target.value, 10))}
          ...
          >
            {comic.chapters.map((ch, i) => (
              <option key={ch.id} value={String(i + 1)}>
                {i + 1}. {ch.title}
              </option>
            ))}
          </select>
